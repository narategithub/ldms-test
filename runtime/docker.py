...
class DockerCluster(LDMSDCluster):
    ...
    def __create(cls, name, image = "centos:7", nodes = 8,
        ...
        if subnet:
            ip_net = ip.ip_network(subnet)
            ip_itr = ip_net.hosts()
        addr4_itr = net.net4.hosts()
        next(addr4_itr) # ditch the '.1' address
        if ipv6:
            addr6_itr = net.net6.hosts()
            next(addr6_itr) # ditch the '.1' address
        ...
        for cl, params in cont_build:
            ...
            if subnet:
                ...
                addr4 = str(next(addr4_itr))
                if first:
                    next(addr4_itr) # skip
                if ipv6:
                    addr6 = str(next(addr6_itr))
                    if first:
                        next(addr6_itr) # skip
                else:
                    addr6 = None
                net.connect(cont, ipv4_address=addr4, ipv6_address=addr6)
            cont.start()
            first = False
