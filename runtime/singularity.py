...
class SContainer(LDMSDContainer):
    ...
    def check_SContainer(self):
        ...
        if self.is_running:
            rc, out = self.exec_run("test -e {}".format(self.CLUSTER_SPEC_PATH))
            if rc:
                raise TypeError("Not a SContainer")
            rc, out = self.exec_run("test -e {}".format(self.NODE_SPEC_PATH))
            if rc:
                raise TypeError("Not a SContainer")
    ...
    def start(self):
        ...
        if self._ip_addr and self._net:
            ...
            ip_addr = "NONE"
        binds = [
                "{}:/opt/ovis".format(G.conf["ldms-test"]["prefix"]),
            ] + self._cluster_spec.get("mounts", [])
